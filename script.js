                        ...
                        data.blog.forEach(post => {
                            ...
                            postDiv.innerHTML = `
                                ...
                                <div class="prose prose-invert max-w-none text-gray-200">
                                    <p>${post.content}</p>
                                    ${post.links ? `<div class="mt-4 space-y-2 text-sm flex flex-col">${post.links.map(link => `<a href="${link.url}" target="_blank" class="text-cyan-400 hover:underline flex items-center"><i class="${link.icon} mr-1"></i>${link.name}</a>`).join('')}</div>` : ''}
                                </div>
                            `;
