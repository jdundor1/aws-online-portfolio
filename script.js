          ...
          data.projects.forEach(project => {
            ...
            if (project.links && project.links.length > 0) {
              ...
              project.links.forEach(link => {
                ...
                projectLinksHtml += `<a href="${linkUrl}" target="_blank" rel="noopener noreferrer" class="text-cyan-400 hover:underline flex items-center mb-2 ${indentClass}"><i class="${link.icon}" aria-hidden="true"></i><span class="ml-1">${link.name}</span></a>`;
              });
...
`;
    ...
    `;
        ...
        if (heroHeadshot) {
          ...
          if (path) {
            const url = /^https?:\/\//i.test(path) ? path : `${s3BaseUrl}${path}`;
            heroHeadshot.src = url;
          }
          heroHeadshot.addEventListener('error', () => {
            console.error('Headshot failed to load:', heroHeadshot.src);
          });
        }
        // <<< END ADDED >>>
